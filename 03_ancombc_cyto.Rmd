...
Run ancombc2 at genus-level
```{r}
# ANCOMBC2 analysis

# Agglomerate by genus and subset by prevalence
tse_cyto_gn <- subsetByPrevalentFeatures(tse_cyto, rank = "Genus", assay.type = "relabundance", prevalence = 5/100, detection = 0.1/100)


results_list_ancombc2_all_cov_gn <- lapply( cytokines, function(cytokine) {
    # Create the formula by pasting the current cytokine
    fix_formula <- paste(cytokine, "+ BL_AGE + MEN + BMI + CURR_SMOKE + PREVAL_DIAB", sep = " ")
...
# Name results lists with corresponding cytokines
names(results_list_ancombc2_all_cov_gn) <- cytokines

# Save results file 
saveRDS(results_list_ancombc2_all_cov_gn, "results_list_ancombc2_all_cov_gn.rds")
...
```{r}
# Clean results and do FDR correction
#results_list_ancombc2_all_cov_gn <- readRDS("results_list_ancombc2_all_cov_gn.rds")
res_ancombc2_all_cov_gn <- lapply(cytokines, function(cytokine) {
  # Extract the 'res' data frame from the corresponding list
  results <- results_list_ancombc2_all_cov_gn[[which(cytokines == cytokine)]]
  res_df <- results$res  
...
# Assign names to the list based on the cytokines
names(res_ancombc2_all_cov_gn) <- cytokines

# Combine all data frames
combined_res_ancombc_all_cov_gn <- do.call(rbind, res_ancombc2_all_cov_gn)
rownames(combined_res_ancombc_all_cov_gn) <- NULL

# Add q_vals 
combined_res_ancombc_all_cov_gn <- combined_res_ancombc_all_cov_gn %>% mutate(q_val = p.adjust(p_value, "BH")) %>% arrange (q_val)

# Save file
saveRDS(combined_res_ancombc_all_cov_gn, "combined_res_ancombc_all_cov_gn.rds")
writexl::write_xlsx(combined_res_ancombc_all_cov_gn, "SupTable10_ancombc_gn.xlsx" )
DT::datatable(combined_res_ancombc_all_cov_gn)

```


Filter for q_val less than 0.05 and TRUE ss
```{r}

results_sp <- readRDS("combined_res_ancombc_all_cov_sp.rds")
results_gn <- readRDS("combined_res_ancombc_all_cov_gn.rds")

results_ancombc_sp <- results_sp %>% filter(q_val < 0.05 & passed_ss == TRUE)
results_ancombc_gn <- results_gn %>% filter(q_val < 0.05 & passed_ss == TRUE)
```
