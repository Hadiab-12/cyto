...
add_covars <- clean_col %>% select(BL_USE_RX_A02BC, TULOT, KOULGR,ALKI2_FR02, Q57X,FID) %>% filter(!is.na(BL_USE_RX_A02BC) & !is.na(TULOT) & !is.na(ALKI2_FR02) & !is.na(Q57X) & !is.na(KOULGR)) %>% inner_join(combined_clr_cyto, by="FID")
  
diff_taxa_add_covars <- expand.grid(Cytokines = sig_cytokines, Species = sig_taxa) %>% 
  mutate(formula = glue::glue("{Cytokines} ~ scale(`{Species}`) + BL_AGE + MEN + BMI + CURR_SMOKE + PREVAL_DIAB + BL_USE_RX_A02BC + TULOT + KOULGR + ALKI2_FR02 + Q57X")) %>%
